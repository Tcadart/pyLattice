    ...
    if name_region == 'AllBeams':  # Traitement all beams
        ...
        if 'X' in name_region:
            edges = e.getByBoundingBox(
                xMin=lattice.xMax - ((lattice.num_cells_x + 1 - couche_number) * lattice.cell_size_x),
                yMin=lattice.yMin, zMin=lattice.zMin,
                xMax=lattice.xMax - ((lattice.num_cells_x - couche_number) * lattice.cell_size_x), yMax=lattice.yMax,
                zMax=lattice.zMax)
            Region = p.Set(edges=edges, name=name_region)
        if 'Y' in name_region:
            edges = e.getByBoundingBox(xMin=lattice.xMin, yMin=lattice.yMax - (
                        (lattice.num_cells_y + 1 - couche_number) * lattice.cell_size_y), zMin=lattice.zMin,
                                       xMax=lattice.xMax, yMax=lattice.yMax - (
                            (lattice.num_cells_y - couche_number) * lattice.cell_size_y), zMax=lattice.zMax)
            Region = p.Set(edges=edges, name=name_region)
        if 'Z' in name_region:
            edges = e.getByBoundingBox(xMin=lattice.xMin, yMin=lattice.yMin, zMin=lattice.zMax - (
                        (lattice.num_cells_z + 1 - couche_number) * lattice.cell_size_z), xMax=lattice.xMax,
                                       yMax=lattice.yMax, zMax=lattice.zMax - (
                            (lattice.num_cells_z - couche_number) * lattice.cell_size_z))
            Region = p.Set(edges=edges, name=name_region)
    return Region
...
def Lattice_geometry_solid(Lattice, Radius_geom):  # Tricks -0.0001 and 1.0001 for trigger cutting part outside cell
    ...
    CubicV2 = [(0.5, -0.0001, 0.5, 0.5, 0.5, 0.5, Radius_geom),
               # Tricks -0.0001 and 1.0001 for trigger cutting part outside cell
               (-0.0001, 0.5, 0.5, 0.5, 0.5, 0.5, Radius_geom),
               (0.5, 1.0001, 0.5, 0.5, 0.5, 0.5, Radius_geom),
               (1.0001, 0.5, 0.5, 0.5, 0.5, 0.5, Radius_geom),
               (0.5, 0.5, -0.0001, 0.5, 0.5, 0.5, Radius_geom),
...
# *******************************************************************************************************************
# Gradient properties
gradDim = gradSettings(GradDimRule, GradDimDirection, GradDimParameters, number_cell_X, number_cell_Y,
                       number_cell_Z)  # sortir des floats
gradRadius = gradSettings(GradRadRule, GradRadDirection, GradRadParameters, number_cell_X, number_cell_Y,
                          number_cell_Z)  # sortir des floats
...
lattice = Lattice(cell_size_X, cell_size_Y, cell_size_Z, number_cell_X, number_cell_Y, number_cell_Z, Lattice_Type,
                  Radius, gradRadius, gradDim, gradMat, MethodSim)
...
if AnalysisType == 1:
    ...
    displacementCompression = (lattice.zMax - lattice.zMin) * compressionPourcent / 100
    Create_Loads_Surface(name_model, 'Step-1', name_surface_Assembly, 'Displacement_surface',
                         [0, 0, -displacementCompression])
    Submit_Job(name_Job, name_model)
    ...
    for i in range(6):
        ...
        number_cell_Y = number_cell
        number_cell_Z = number_cell
        generatedModel = Kernel.Library.Lattice.generateGeneralLattice(name_model,
                                                                       (cell_size_X, cell_size_Y, cell_size_Z),
                                                                       Lattice_geom,
                                                                       (number_cell_X, number_cell_Y, number_cell_Z),
                                                                       Maillage_size)
        ...
        delta = 0.1
        faces = f1.getByBoundingBox(xMin=-delta, yMin=-delta, zMin=cell_size_Z * number_cell_Z - delta,
                                    xMax=cell_size_X * number_cell_X + delta, yMax=cell_size_Y * number_cell_Y + delta,
                                    zMax=cell_size_Z * number_cell_Z + delta)
        ...
        mdb.models[name_model].DisplacementBC(name='Displacement',
                                              createStepName='StaticPlastic', region=region, u1=0.0, u2=0.0,
                                              u3=-displacement,
                                              ur1=UNSET, ur2=UNSET, ur3=UNSET, amplitude=UNSET, fixed=OFF,
                                              distributionType=UNIFORM, fieldName='', localCsys=None)
        faces = f1.getByBoundingBox(xMin=-delta, yMin=-delta, zMin=-delta, xMax=cell_size_X * number_cell_X + delta,
                                    yMax=cell_size_Y * number_cell_Y + delta, zMax=+delta)
